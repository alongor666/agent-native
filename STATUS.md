...
- 第 2 个独立项目 `../preknow_shanxi`（组织人员分析 + 知识库，nfra 的消费方）：实测 **L0**，sat=[F,T,T,F]——②③全绿却因无数据集版本兼容声明（A1.3）卡在公理①、停 L0（前缀规则"跳级不升级"实例）。**已为其补 A1.3**（数据集版本清单 dataset_manifest.json + check 门禁，跨仓 PR#25）→ 公理①全满，**升 L1**。
...
9. [ ] **升 Stable（1.0.0）**：§2 退出条件满 2/3——无 MAJOR 争议 ✓ + ≥2 独立项目自评 ✓（nfra L2 / preknow L1，均经修复达标）；**待**公开反馈期 ≥ 一迭代周期无阻断异议（需外部采用/反馈，非本地可即时完成）。
